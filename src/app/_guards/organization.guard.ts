...
import { VOLUNTARY_FALLBACK_PAGE_URI, ADMINISTRATOR_FALLBACK_PAGE_URI } from './tokens';
...
export class OrganizationGuard implements CanActivate {
  ...
  canActivate(
    ...
    const isOrganization = localStorage.getItem('role') === 'Organization';
    if (!isAuthenticated || !isOrganization) {
      const role = localStorage.getItem('role');
      swal({
        title: 'Acceso Denegado',
        text: 'Solo puedes acceder a esta pagina si eres una organización \n has sido redirigido a tu inicio',
        type: 'error',
      });
      switch (role) {
        case 'Administrator':
